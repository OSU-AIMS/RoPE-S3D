...
class Predictor():
    def __init__(self,
        ...
        with h5py.File('SLU30.h5','r') as f:
    ...
    def run(self, og_image, target_img, target_depth, camera_pose = None, starting_point = None):
        ...
        if starting_point is None:
            ...
            stages = [lookup, u_stage, s_flip_check_6, slu_fine_tune]
            stages = [lookup, slu_fine_tune]
...
class DoubleLookupPredictor():
    def __init__(self,
        ...
        with h5py.File('SL75.h5','r') as f:
            self.lookup_angles_a = np.copy(f['angles'])
            self.lookup_depth_a = tf.pow(tf.constant(np.copy(f['depth']),tf.float32),0.5)

        with h5py.File('SLU50.h5','r') as f:
            self.lookup_angles_b = np.copy(f['angles'])
            self.lookup_depth_b = np.copy(f['depth'])
    ...
    def run(self, og_image, target_img, target_depth, camera_pose = None, starting_point = None):
        ...
        if starting_point is None:
            ...
            lookup_a = ['lookup_a']
            lookup_b = ['lookup_b', [0.25,0.25,None,None,None,None]]
            u_sweep = ['smartsweep', 25, 6, None, [False,False,True,False,False,False]]
            u_stage = ['descent',30,6,0.5,.1,[False,False,True,False,False,False],[0.1,0.1,0.4,0.5,0.5,0.5]]
            s_flip_check_6 = ['flip', 6, [True,False,False,False,False,False]]
            slu_fine_tune = ['descent',10,6,0.4,.015,[True,True,True,False,False,False],[None,None,None,None,None,None]]
            ...
            #stages = [lookup, u_stage, s_flip_check_6, slu_fine_tune]
            stages = [lookup_a, lookup_b, slu_fine_tune]
        ...
        for stage in stages:
            ...
            if stage[0] == 'lookup_a':
                ...
                stack = tf.stack([tf.pow(tf.constant(self._tgt_depth, tf.float32),0.5)]*len(self.lookup_angles_a))

                diff = stack - self.lookup_depth_a
                diff = tf.abs(diff) 
                lookup_err = tf.reduce_mean(diff, (1,2)) *- tf.math.reduce_std(diff, (1,2))

                angles = self.lookup_angles_a[tf.argmin(lookup_err).numpy()]

            if stage[0] == 'lookup_b':

                expand = True
                ranges = np.array(stage[1])
                angs = np.copy(self.lookup_angles_b[:,ranges != None])
                angs_full = np.copy(self.lookup_angles_b)

                while expand:
                    range_vals = ranges[ranges != None]

                    upper = np.min([(angles[ranges != None] + range_vals), self.u_reader.joint_limits[:,1][ranges != None]],-1)
                    lower = np.max([(angles[ranges != None] - range_vals), self.u_reader.joint_limits[:,0][ranges != None]],-1)

                    filter = np.all(angs >= lower,-1) * np.all(angs <= upper,-1)
                    if sum(filter) > 0:
                        expand = False
                    else:
                        ranges[ranges != None] += 0.1

                angs = angs[filter]
                angs_full = np.copy(self.lookup_angles_b)[filter]

                stack = tf.stack([tf.pow(tf.constant(self._tgt_depth, tf.float32),0.5)]*len(angs))
                depth = self.lookup_depth_b[filter]
                depth = tf.pow(tf.constant(depth,tf.float32),0.5)

                diff = stack - depth
                diff = tf.abs(diff) 
                lookup_err = tf.reduce_mean(diff, (1,2)) *- tf.math.reduce_std(diff, (1,2))

                angles = angs_full[tf.argmin(lookup_err).numpy()]
