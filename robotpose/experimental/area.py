...
class AreaMatcherStagedZonedError():
    ...
    def run(self, og_image, target_img, target_depth, camera_pose = None, starting_point = None):
        ...
        if starting_point is None:
            ...
            s_sweep_1 = ['sweep', 10, 2, [True,False,False,False,False,False]]
            l_sweep_1 = ['sweep', 15, 3, [False,True,False,False,False,False]]
            s_sweep_2 = ['sweep', 10, 3, [True,False,False,False,False,False]]
            l_sweep_2 = ['sweep', 15, 4, [False,True,False,False,False,False]]
            s_flip_check_3 = ['flip',4,[True,False,False,False,False,False]]
            sl_rough = ['descent',15,4,0.7,.2,[True,True,False,False,False,False],[0.8,0.5,0.075,0.5,0.5,0.5]]
            u_sweep = ['sweep', 20, 6, [False,False,True,False,False,False]]
            slu_stage_1 = ['descent',15,6,0.5,.1,[True,True,True,False,False,False],[None,None,None,None,None,None]]
            ...
            stages = [s_sweep_1, l_sweep_1, s_sweep_2, l_sweep_2, s_flip_check_3, sl_rough, s_flip_check_3, u_sweep, slu_stage_1, slu_stage_2, s_flip_check_6, lu_fine_tune]
        else:
    ...
    def _mask_err(self, seg_data, num_joints, render):
        ...
        for link in self.link_names[:num_joints]:
            if link in seg_data.keys():
                count += 1
                joint_mask = seg_data[self.link_names[self.link_names[:num_joints].index(link)]]['mask']

                render_mask = np.zeros(render.shape[0:2], dtype=bool)
                render_mask[np.where(np.all(render == color_dict[link], axis=-1))] = True
    ...
    def _total_err(self, num_joints, render_color, render_depth):
        return self._depth_err(num_joints,render_depth,render_color) #+ self._mask_err(seg_data,num_joints,render_color)
