...
"""
Types:
    - Revolve: Camera moves around a center point it always faces. Can vary center point
    - Grid: Camera moves laterally in a 3D grid.

"""

class CameraLookupCreator(Renderer):
    def __init__(self, robot_pose: np.ndarray, intrinsics: Union[str, Intrinsics]):
        self.robot_pose = robot_pose
        self.u_reader = URDFReader()
        super().__init__('seg', camera_intrin=intrinsics)

    def load_config(self, angles_to_do: np.ndarray, divisions:np.ndarray):
 
        self.setMaxParts(None)
        self.divisions = np.array(divisions)
